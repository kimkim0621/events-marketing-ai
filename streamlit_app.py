...
def show_main_interface():
    ...
    with st.expander("👥 職種選択 (31職種)", expanded=True):
        ...
        st.session_state.selected_job_titles = selected_job_titles
    
    with st.expander("🏢 従業員規模選択 (8段階)", expanded=True):
        ...
        st.session_state.selected_company_sizes = selected_company_sizes
    ...
    with col1:
        target_attendees = st.number_input("目標参加者数", min_value=1, max_value=10000, value=100)
        budget = st.number_input("予算（万円）", min_value=0, max_value=1000, value=50)
    
    with col2:
        event_date = st.date_input("イベント開催日", value=datetime.now().date())
    ...
    if st.button("🎯 施策提案を生成", type="primary"):
        if event_name and event_theme and selected_industries and selected_job_titles and selected_company_sizes:
            with st.spinner("施策提案を生成中..."):
                try:
                    ...
                    # 結果を表示
                    st.success("施策提案を生成しました！")
                    st.json(recommendations)
                    
                except Exception as e:
                    st.error(f"施策提案生成エラー: {str(e)}")
        else:
